...
#define TIMER_ACTION_MIMIC_PIN 25
...
void setup() {
  ...
  pinMode(TIMER_ACTION_MIMIC_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  if ((currentProgramTime >= latestServoActive + 10)) {
    ...
    // Ref: https://github.com/Dlloydev/ESP32-ESP32S2-AnalogWrite#description-1
    if (isServoOn) {
      // Move when signal is active
      // ye1 = myservo.write(SERVO_SIGNAL_PIN, 85);
      // ye1 = myservo.write(SERVO_SIGNAL_PIN, 38, 5.0, 0.8);
      ye1 = myservo.write(SERVO_SIGNAL_PIN, 10);
      isSpin = false;

    } else {
      // Set to no movement (90)
  ...
  // Check almost every seconds
  if (currentProgramTime >= latestTimerActiveTime + 1200) {
    bool timerActionMimicPinRead = digitalRead(TIMER_ACTION_MIMIC_PIN);
    Serial.print("isTimerActive:");
    Serial.println(isTimerActive);
    Serial.print("prevTimerActive:");
    Serial.println(prevTimerActive);
    Serial.print("timerActionMimicPinRead:");
    Serial.println(timerActionMimicPinRead == HIGH ? "HIGH" : "LOW");

    latestTimerActiveTime = currentProgramTime;
    // isTimerActive = false;
    isTimerActive = digitalRead(TIMER_ACTION_MIMIC_PIN) == HIGH;
  }

  if (currentProgramTime >= latestPinCheckTime + 10) {
    latestPinCheckTime = currentProgramTime;
    isCutoffActive = digitalRead(SERVO_CUTOFF_PIN) == LOW; // Default is LOW
    ...
    isServoOn = isManualOnSwitchActive || isTimerActive;
  }
